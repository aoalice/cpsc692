...
sds = pickle.load(open("./sds", "rb"), encoding='latin1') # note for later: had to change to latin1 encoding, not sure of its effects
series = pickle.load(open("./series", "rb"), encoding='latin1')
...
'''
my edited code for saliency only
ds = np.zeros(shape=(N, 1, H, W)) # is N # of images or number of frames?
ds[:, 0, :, :] = sds
'''

Xtest, ytest = create_dataset(ds, look_back=look_back, look_ahead=look_ahead)
ytest = series[-len(Xtest):] # note-to-self: what is ytest even used for?
